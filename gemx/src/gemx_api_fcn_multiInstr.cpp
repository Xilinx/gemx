...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i < GEMX_numKernels; i++) {
        ...
        for(int index = 0; index<l_instrCount; index++){
            if(readFromFiles){             
              l_argIdx=l_argIdx+3;
              l_insFileName[index] = argv[l_argIdx++];
              l_matAFileName[index] = argv[l_argIdx++];
              l_matBFileName[index] = argv[l_argIdx++];
              l_matXFileName[index] = argv[l_argIdx++];
              l_fcn.addInstrFromFiles(index,l_program[i], l_insFileName[index],l_matAFileName[index],l_matBFileName[index],l_matXFileName[index],false);
            }else{
               ...
               if (! l_fcn.check(l_m[index], l_k[index], l_n[index], l_lda[index], l_ldb[index], l_ldc[index], l_ldx[index])) {
                  return EXIT_FAILURE;
               }

              l_fcn.addInstr(l_program[i], l_m[index], l_k[index], l_n[index], l_lda[index], l_ldb[index], l_ldc[index], l_ldx[index], l_postScale[index],l_PReluVal[index],
                             l_handleA[index], l_handleB[index], l_handleC[index], l_handleX[index], false);
            }
        }
    }
    //golden program
    ...
    if(!getenv("SKIPPED_GOLD_CAL")){
        for(int index = 0; index<l_instrCount; index++){
            if(readFromFiles){
              l_fcn.addInstrFromFiles(index,l_program_golden, l_insFileName[index],l_matAFileName[index],l_matBFileName[index],l_matXFileName[index],true);

            }else{
               l_fcn.addInstr(l_program_golden,  l_m[index], l_k[index], l_n[index], l_lda[index], l_ldb[index], l_ldc[index], l_ldx[index], l_postScale[index],l_PReluVal[index],
                             l_handleA[index], l_handleB[index], l_handleC[index], l_handleX[index], true);
            }
    ...
    if (readFromFiles){
      ...
      for(int i=0;i<l_instrCount;++i){
          ...
          << l_maxCycleCount[i] << "," << l_maxTimeKernelInMs[i] << "," << l_timeApiInMs
          << std::endl;
      }
