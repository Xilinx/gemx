...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i < GEMX_numKernels; i++) {
        ...
        for(int index = 0; index<l_instrCount; index++){
            ...
            if (! l_gemm.check(l_m[index], l_k[index], l_n[index], l_lda[index], l_ldb[index], l_ldc[index], l_ldx[index])) {
                return EXIT_FAILURE;
            }
            l_gemm.addInstr(l_program[i], l_m[index], l_k[index], l_n[index], l_lda[index], l_ldb[index], l_ldc[index], l_ldx[index], l_postScale[index],
    ...
    if(!getenv("SKIPPED_GOLD_CAL")){
        for(int index = 0; index<l_instrCount; index++){
            l_gemm.addInstr(l_program_golden, l_m[index], l_k[index], l_n[index], l_lda[index], l_ldb[index], l_ldc[index], l_ldx[index], l_postScale[index],
