...
class Fpga
{
  ...
  public:
    ...
    bool
    createKernel(unsigned int p_kernelId, std::string p_kernelName) {
      bool ok = false;
      assert(p_kernelId < GEMX_numKernels);
      cl::Kernel l_kernel(m_Program, p_kernelName.c_str());
    ...
    bool 
    createBufferForKernel(unsigned int p_kernelId, MemDesc p_memDesc) {
      bool ok = false;
      
      assert(p_kernelId < GEMX_numKernels);
      cl::Buffer l_buffer(m_Context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, p_memDesc.sizeBytes(), p_memDesc.data());
