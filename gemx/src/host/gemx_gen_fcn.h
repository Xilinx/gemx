...
*/
#ifndef GEMX_GEN_FCN_H
#define GEMX_GEN_FCN_H 

#include "gemx_gen_bin.h"
#include "gemx_matrix.h"

typedef FcnType::FcnArgsType FcnArgsType;
typedef DenseMat<GEMX_XdataType> XMatType;

template <typename T>
void fcn_ref(DenseMat<T> & p_A, DenseMat<T> & p_B, DenseMat<T> & p_C, DenseMat<GEMX_XdataType> & p_X,  int32_t p_postScale, int16_t p_PReluVal) {
...
class GenFcn
{
  public:
    bool checkDim(
      std::string p_VarName, unsigned int p_Val, unsigned int p_Mod, unsigned int p_Min
    ) {
        bool l_ok = true;
        if (p_Val % p_Mod != 0) {
    ...
    bool check(
      ...
      unsigned int p_LdX
        ...
        const unsigned int l_mMin = GEMX_gemmMBlocks * GEMX_ddrWidth;
        const unsigned int l_nMin = GEMX_gemmNBlocks * GEMX_ddrWidth;
        const unsigned int l_kMin = GEMX_gemmKBlocks * GEMX_ddrWidth; // GEMX_gemmKBlocks must >= 2, due to kernel Cout control to save area
        
        ok = checkDim("M", p_M, l_mMin, l_mMin) &&
             checkDim("K", p_K, l_kMin, l_kMin) &&
    ...
    void addInstrFromFile(
      ...
      bool p_WithGolden
        ...
        if (l_good) {
          ...
          if (p_WithGolden){
            fcn_ref<GEMX_dataType>(l_matA, l_matB, l_matC, l_matX, l_postScale, l_PRelu);
          }
    ...
    void addInstr(
      ...
      std::string p_handleA, std::string p_handleB, std::string p_handleC,
      ...
      bool p_WithGolden
        ...
        if (p_WithGolden) {
            fcn_ref<GEMX_dataType>(l_matA, l_matB, l_matC, l_matX, p_postScale, p_PReluVal);
        }
